...
GITHUB_TOKEN = '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'
...
async def genkey(message):
    ...
    try:
        ...
        status, resp_text = await update_file_content(
            "auth_list.json",
            auth_list,
            sha,
        ...
        if status not in (200, 201):
            await bot.reply_to(message, f"❌ GitHub upload မအောင်မြင်ပါ (Status: {status})")
            return
        await bot.reply_to(
            message,
            f" Key Generated\n\n"
